...
export function BottleExperience({onReady}:{onReady:()=>void}) {
  ...
  return <section id="top" className={`experience phase-${phase} ${active?"is-rotating":""} ${auto?"is-auto":""}`} style={{"--turn":turn} as React.CSSProperties} aria-label="Shrotas bottle introduction">
    <div className="atmosphere"/><div className="hero-type" aria-hidden><span>THE ART</span><span>OF</span><span>HYDRATION</span></div><div className="orbit-copy orbit-back" aria-hidden>THE ART OF HYDRATION</div><div className="orbit-copy orbit-front" aria-hidden>THE ART OF HYDRATION</div><div className="hero-foretype" aria-hidden>SHROTAS</div>
