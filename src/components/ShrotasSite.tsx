...
export function ShrotasSite(){
  ...
  return <main ref={root} className={entered?"site-entered":""}><HeroScene onReady={ready}/>
    ...
    <footer id="contact" className="scene finale"><Image src={assets.heroFront} alt="Shrotas bottle" width={768} height={1024}/><div data-reveal><p className="eyebrow">SHROTAS</p><h2>The Art<br/>of Hydration.</h2><p>{f.category} · {f.volume}</p><div className="finale-contact"><a href={businessContent.contact.phoneHref}>{businessContent.contact.phone}</a><a href={businessContent.contact.emailHref}>{businessContent.contact.email}</a></div></div><small>© {new Date().getFullYear()} SHROTAS</small></footer>
  </main>;
}
