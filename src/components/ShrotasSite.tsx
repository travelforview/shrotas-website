...
export function ShrotasSite(){
  ...
  return <main ref={root} className={entered?"site-entered":""}><HeroScene onReady={ready}/>
    ...
    <section id="bottle" className="product-study"><div className="study-stage"><div className="stage-number">0{angle+1}</div>{studies.map((s,i)=><div key={s.src} className={`study-image ${i===angle?"is-active":""}`}><Image src={s.src} alt={`${s.kicker} view of the Shrotas bottle`} fill sizes="(max-width: 760px) 65vw, 38vw"/>{s.address&&<><span className="study-address-patch owner"><Image src={assets.addressLabel} alt="Ground, G 14, Ashapura Complex, Silvassa Vapi Road, Valsad, Gujarat, 396195, Valsad, Gujarat-396195" width={201} height={35}/></span><span className="study-address-patch packer" aria-hidden><Image src={assets.addressLabel} alt="" width={201} height={35}/></span></>}</div>)}<div className="study-caption"><p>{active.kicker}</p><h2>{active.title}</h2><span>{active.body}</span></div></div><div className="study-track" aria-label="Product facts">{studies.map((s,i)=><article className="study-step" key={s.src}><span>0{i+1}</span><h3>{s.title}</h3><p>{s.body}</p></article>)}</div></section>
