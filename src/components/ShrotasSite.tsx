...
const studies=[
  {src:assets.front,kicker:"Composition / 01",title:"Essential Minerals",body:f.minerals.join(" · "),address:false},
  {src:assets.side,kicker:"Origin / 02",title:"Source to Soul",body:"What begins as water becomes Shrotas.",address:true},
  {src:assets.back,kicker:"Material / 03",title:"Designed to Recycle",body:f.recyclable,address:false},
];
...
export function ShrotasSite(){
  ...
  return <main ref={root} className={entered?"site-entered":""}><HeroScene onReady={ready}/>
    ...
    <section id="bottle" className="product-study"><div className="study-stage"><div className="stage-number">0{angle+1}</div>{studies.map((s,i)=><div key={s.src} className={`study-image ${i===angle?"is-active":""}`}><Image src={s.src} alt={`${s.kicker} view of the Shrotas bottle`} fill sizes="(max-width: 760px) 65vw, 38vw"/>{s.address&&<><Image className="study-address-overlay owner" src={assets.addressLabel} alt="Ground, G 14, Ashapura Complex, Silvassa Vapi Road, Valsad, Gujarat, 396195, Valsad, Gujarat-396195" width={201} height={35}/><Image className="study-address-overlay packer" src={assets.addressLabel} alt="" aria-hidden width={201} height={35}/></>}</div>)}<div className="study-caption"><p>{active.kicker}</p><h2>{active.title}</h2><span>{active.body}</span></div></div><div className="study-track" aria-label="Product facts">{studies.map((s,i)=><article className="study-step" key={s.src}><span>0{i+1}</span><h3>{s.title}</h3><p>{s.body}</p></article>)}</div></section>
